...
class StreamManager {
    ...
    setupEventListeners() {
        ...
        if (subtitlesBtn) subtitlesBtn.addEventListener('click', () => this.toggleSubtitles());
    ...
    createContentItem(showId, show, season = null, episode = null) {
        ...
        item.innerHTML = `
            ...
            <div class="content-overlay">
                <h3>${show.title}</h3>
                <p>${show.year} ??? ${show.genre} ??? ${show.seasons} Season${show.seasons > 1 ? 's' : ''}</p>
            </div>
        `;
                ...
                <div class="episode-item${isActiveEpisode ? ' active watching' : ''}" data-episode="${episodeNum}">
                    <div class="episode-card">
                        ...
                        <div class="episode-card-inner">
                            ...
                            <div class="ep-sub">${episode.duration} ??? ${episode.description || ''}</div>
                        </div>
                    </div>
                </div>
            `;
